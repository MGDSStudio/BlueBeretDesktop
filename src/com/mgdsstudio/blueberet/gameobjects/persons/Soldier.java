...
public class Soldier extends Human implements IDrawable {

	private SpritesheetChangingController spritesheetChangingController;
	private ArrayList<WeaponType> deblockedWeapons;
	public final static String CLASS_NAME = "Soldier";
	private final String objectToDisplayName = "Player";
	...
	public Soldier(PVector position, GameRound gameRound) {
		...
		makeDeadBody();
		if (Program.debug) System.out.println("Soldier was uploaded");
	...
	public void recalculateJumpVelocity(boolean bulletTime) {
		...
		jumpStartSpeed = startSpeed;
		System.out.println("This function is not adusted to the real player bullet time coef");
		//System.out.println("Jump start speed was set on " + jumpStartSpeed + " by mass: " + body.getMass() + "; and scale " + body.getGravityScale());
		/*
		jumpMaxHeight = 20;
		float startSpeed = PApplet.sqrt(jumpMaxHeight * PApplet.abs(2 * PhysicGameWorld.GRAVITY));
		jumpStartSpeed = startSpeed;
	...
	public void attacked(int damageValue) {
		if (!underAttack) {
			...
			System.out.println("Soldier is attacked");
		}
	}
